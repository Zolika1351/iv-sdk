...
public:
	...
	{
		return ((float(__thiscall*)(CDraw*, bool))(Addresses::nCalculateAspectRatio))(this, bUnk);
	}
};
VALIDATE_OFFSET(CDraw, m_pRenderPhases, 0x38);
VALIDATE_OFFSET(CDraw, m_bForceHudWidescreen, 0x453);

CDraw& Scene = *(CDraw*)nullptr; // scene? renderer? rw to rage wrapper? some core rage rendering class? just called it CDraw, Scene & GlobalScene for now